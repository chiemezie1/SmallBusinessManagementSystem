...
static const char UnityStrErrFloat[]               = "Unity Floating Point Disabled";
static const char UnityStrErrDouble[]              = "Unity Double Precision Disabled";
static const char UnityStrErr64[]                  = "Unity 64-bit Support Disabled";
                                           ...
                                           const UNITY_DISPLAY_STYLE_T style)
{
    ...
    if ((compare & UNITY_GREATER_THAN) && (actual < threshold)) { failed = 1; }
    if ((compare & UNITY_SMALLER_THAN) && (actual > threshold)) { failed = 1; }
                              ...
                              const UNITY_FLAGS_T flags)
{
    ...
    while ((elements > 0) && (elements--))
    {
        ...
        {
            expected = (UNITY_INTERNAL_PTR)((_UP)expected + length);
        }
        actual   = (UNITY_INTERNAL_PTR)((_UP)actual + length);
    }
}
...
int UnityStringArgumentMatches(const char* str)
{
    ...
    while (ptr1[0] != 0)
    {
        if (ptr1[0] == '"')
        {
            ...
            if (ptrf == 0)
            {
                UnityPrint("Error: Second Quote Not Found");
                UNITY_PRINT_EOL();
                return 0;
            }
        }
        else
        {
            ...
            ptrf = strchr(ptr2, ',');
            if (ptrf == 0)
                ptrf = ptr2 + strlen(ptr2);
        }

        while ((ptr2[0] != 0) && (ptr2 < ptrf))
        {
            if (UnityOptionIncludeNamed)
            {
                retval = IsStringInBiggerString(Unity.TestFile, ptr2);
                if (retval == 1)
                    return 1;
                if (retval == 2)
                    return 0;
...
int UnityTestMatches(void)
{
    ...
    if (UnityOptionIncludeNamed)
    {
        retval = IsStringInBiggerString(Unity.TestFile, UnityOptionIncludeNamed);
        if (retval == 0)
            return 0;
    }
    ...
    if (UnityOptionExcludeNamed)
    {
        retval = IsStringInBiggerString(Unity.TestFile, UnityOptionExcludeNamed);
        if (retval == 1)
            return 0;
    }
    return 1;
